...
class StableDiffusion(nn.Module):
    ...
    def refine(self, pred_rgb,
        ...
        # loss = 0.5 * F.mse_loss(imgs.float(), pred_rgb_512, reduction='sum') / latents.shape[0]
        # return loss
        return imgs

    def refine_step(self, pred_rgb,
                    guidance_scale=100, steps=50, strength=0.8, ):
        pred_rgb_512 = F.interpolate(pred_rgb, (512, 512), mode='bilinear', align_corners=False)
        imgs = self.refine(pred_rgb,
                           guidance_scale=guidance_scale, steps=steps, strength=strength)

        loss = 0.5 * F.mse_loss(imgs.float(), pred_rgb_512, reduction='sum') / imgs.shape[0]
        return loss
