    ...
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        ...
        if specular:
            ...
            if not hasattr(self, '_FG_LUT'):
                self._FG_LUT = torch.as_tensor(np.fromfile('dataset/realengine/irrmaps/bsdf_256_256.bin', dtype=np.float32).reshape(1, 256, 256, 2), dtype=torch.float32, device='cuda')
            fg_lookup = dr.texture(self._FG_LUT, fg_uv, filter_mode='linear', boundary_mode='clamp')

            # Roughness adjusted specular env lookup
            miplevel = self.get_mip(roughness)
            spec = dr.texture(self.specular[0][None, ...], reflvec.contiguous(), mip=list(m[None, ...] for m in self.specular[1:]), mip_level_bias=miplevel[..., 0], filter_mode='linear-mipmap-linear',
                              boundary_mode='cube')
...
class EnvironmentLight_Lat(torch.nn.Module):
    LIGHT_MIN_RES = 16

    MIN_ROUGHNESS = 0.08
    MAX_ROUGHNESS = 0.5

    def __init__(self, cube_res=512, img_size=(512, 1024), num_gaussians=50, device="cuda", hdr_map=None, lgt_intensity=0.1):
        super(EnvironmentLight_Lat, self).__init__()
        self.mtx = None
        self.res = cube_res
        ...
        xy = (torch.rand(num_gaussians, 2, device=device) - 0.5) * 1.2 + 0.5
        # xy[:,1] = 0.5
        self.hdr_map = hdr_map
        self.ano_env_light = None
        # 可学习参数
        self.positions = nn.Parameter(xy)  # (x, y) in normalized coordinates [0, 1]
        self.scales = nn.Parameter(torch.ones(num_gaussians, 2, device=device) * (-2.5))  # Scale for (x, y)
        self.thetas = nn.Parameter(torch.zeros(num_gaussians, device=device) * 2 * math.pi)  # Rotation angles
        self.alphas = nn.Parameter(torch.zeros(num_gaussians, device=device) - 1)  # Alpha weights
        self.color = nn.Parameter(torch.ones(num_gaussians, 3, device=device))
        color_scale = self.base.mean()
        # self.color = nn.Parameter(torch.ones(num_gaussians,3, device=device)/color_scale * 0.25) ## for snerf++
        self.color = nn.Parameter(torch.ones(num_gaussians, 3, device=device) / color_scale * lgt_intensity)  ## for streetgaussian

        # self.to_opengl = torch.tensor([[1, 0, 0], [0, 0, 1], [0, -1, 0]], dtype=torch.float32, device="cuda")
        self.to_opengl = torch.tensor([[1, 0, 0], [0, 1, 0], [0, 0, 1]], dtype=torch.float32, device="cuda")
    ...
    def render(self):
        ...
        color = torch.relu(self.color)
        gaussians = torch.exp(-((x_rot / sx) ** 2 + (y_rot / sy) ** 2))  # Shape: (num_gaussians, H, W)
        gaussian_color = gaussians.unsqueeze(-1) * color.reshape(self.num_gaussians, 1, 1, 3)

        # Combine Gaussians multiplicatively
        color_map = torch.sum(alpha[..., None] * gaussian_color, dim=0)  # Shape: (H, W)
    ...
    @property
    def base(self):
        if self.hdr_map is not None:
            return self.hdr_map
        base = self.render()
        cubemap = util.latlong_to_cubemap(base, res=(self.res, self.res))
        if self.ano_env_light is not None:
            cubemap_2 = self.ano_env_light.base
            cubemap = cubemap + cubemap_2
        return cubemap
        ...
        if specular:
            ...
            if not hasattr(self, '_FG_LUT'):
                self._FG_LUT = torch.as_tensor(
                    np.fromfile('dataset/realengine/irrmaps/bsdf_256_256.bin',
                                dtype=np.float32).reshape(1, 256, 256, 2),
...
def load_env_lat(fn, scale=1.0):
    if os.path.splitext(fn)[1].lower() == ".hdr":
        latlong_img = torch.tensor(util.load_image(fn), dtype=torch.float32, device='cuda') * scale
        cubemap = util.latlong_to_cubemap(latlong_img, [512, 512])
        l = EnvironmentLight_Lat(hdr_map=cubemap)
        l.build_mips()
        return l
    else:
        assert False, "Unknown envlight extension %s" % os.path.splitext(fn)[1]
...
def save_env_map_png(fn, light):
    # assert isinstance(light, EnvironmentLight), "Can only save EnvironmentLight currently"
    # if isinstance(light, EnvironmentLight):
    if len(light.base.shape) == 4:
        color = util.cubemap_to_latlong(light.base, [512, 1024]).clone().detach()
    else:
        color = light.base.clone().detach()

    channel_max = color.amax(dim=(0, 1), keepdim=True)
    save_image((color / channel_max).permute(2, 0, 1), fn)
