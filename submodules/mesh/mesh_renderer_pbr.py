...
def run(opt, render_fn=None, rast=None, ST=None, size=(512, 512)):
    # clear gpu memory
    rgb = None
    grad = None
    vis_grad = None
    ...
    target = None
    mode = "rgb"
    guidance = StableDiffusion("cuda", opt.fp16, opt.vram_O, opt.sd_version, opt.hf_key, t_range=[0.02, 0.5])
    with torch.no_grad():
        torch.cuda.empty_cache()

    num_images = len(bg_paths)
    bg_rgbs = []
    for bg_path in bg_paths:
        img = cv2.imread(bg_path, cv2.IMREAD_UNCHANGED)
        img = cv2.cvtColor(img[..., :3], cv2.COLOR_BGR2RGB) / 255
        bg_rgb = torch.tensor(img, dtype=torch.float32, device="cuda")[None, ...]
        bg_rgb = downsample(bg_rgb, size)
        bg_rgbs.append(bg_rgb)

    if rast is not None:
        for param in rast.parameters():
            param.requires_grad_()
        pbr_params = [{'params': rast.parameters(), 'lr': opt.lr / 2}]
    else:
        pbr_params = []
    ...
    if ST is not None:
        for param in ST.parameters():
            param.requires_grad_()
        pbr_params = pbr_params + [{'params': ST.parameters(), 'lr': opt.lr / 5}]

    # import pdb; pdb.set_trace()
    # optimizer = torch.optim.AdamW(([
    #                                    {'params': color_map, 'lr': opt.lr},  # 模型1的学习率
    #                                    {'params': shadow_map.parameters(), 'lr': opt.lr / 2}  # 模型2的学习率
    #                                ] + pbr_params), weight_decay=0, betas=(0.9, 0.999))
    optimizer = torch.optim.AdamW(pbr_params, weight_decay=0, betas=(0.9, 0.999))

    ### add model
    # renderer.lgt.ano_env_light = st.env_light

    # optimizer = torch.optim.Adam([target], lr=opt.lr, weight_decay=0)
    num_steps = opt.steps
    # scheduler = get_cosine_schedule_with_warmup(optimizer, 100, int(num_steps * 3))
    scheduler = None
    scaler = torch.cuda.amp.GradScaler(init_scale=2. ** 10, enabled=opt.fp16)
    guidance.get_text_embeds([opt.prompt], [""])
    ...
    def compose_image():
        ...
        composed_rgb = bg_rgbs[idx] + 0  # b,c,h,w

        ST.precompute_rayintersect(resize, plane_normal, plane_point, c2car_ls[idx][0], intrinsic_re[0], valid_mask=None)
        alpha_shadow = ST.render_shadow()
        alpha_shadow = alpha_shadow.reshape(1, *size, 3).permute(0, 3, 1, 2)

        shadow_mask = torch.ones_like(composed_rgb)
        composed_rgb = composed_rgb * (1 - shadow_mask) + shadow_mask * (composed_rgb * alpha_shadow)

        render_results = render_fn(c2car_ls[idx])
        fg_mask = render_results['mask']
        fg_rgb = render_results['shaded'][..., :3]
        fg_mask = downsample(fg_mask)
        fg_rgb = downsample(fg_rgb)
    ...
    try:
        for step in tqdm(range(num_steps)):
            ...
            with torch.cuda.amp.autocast(enabled=opt.fp16):
                # import pdb; pdb.set_trace()
                composed_rgb = compose_image()
                loss = guidance.train_step(composed_rgb, guidance_scale=opt.cfg)
                # loss = guidance.refine_step(composed_rgb, guidance_scale=opt.cfg)
            ...
            if step % 10 == 0:
                # composed_rgb = bg_rgb+0
                # # composed_rgb[shadow_region] = composed_rgb[shadow_map]*shadow_map
                # composed_rgb[fg_mask] = (fg_rgb*torch.sigmoid(color_map))[fg_mask]*2
                composed_rgb = compose_image()

                rgb = composed_rgb
                img_rgb = rgb.clamp(0, 1).detach().squeeze(0).cpu().numpy()
                img = (img_rgb * 255).astype(np.uint8)
                cv2.imwrite(os.path.join(opt.save_path, f'{step:04d}.png'),
                            cv2.cvtColor(img.transpose(1, 2, 0), cv2.COLOR_RGB2BGR))
                if rast is not None:
                    # light.save_env_map_png(
                    #     os.path.join(opt.save_path, f'{step:04d}_env.png'), rast.lgt)
                    light.save_env_map(os.path.join(opt.save_path, f'{step:04d}_env.hdr'), rast.lgt)
                if ST is not None:
                    # light.save_env_map_png(
                    #     os.path.join(opt.save_path, f'{step:04d}_env_ST.png'), ST.env_light)
                    light.save_env_map(os.path.join(opt.save_path, f'{step:04d}_env_ST.hdr'), ST.env_light)
...
if __name__ == '__main__':
    ...
    opt.prompt = "A city street in the morning, inserted a car"  # "A wide city street at noon, inserted a police car "
    token = "b1a87fffaada51de"
    ...
    traj_file = 'b1a87fffaada51de_hard_01'  # 'b2ad937212f85714_00'
    vehicle = 'vehicle_8'  # 'vehicle_1'
    lgt_intensity = 1

    final_iter = None  # 500

    opt.save_path = os.path.join(opt.save_path, traj_file, vehicle)
    ...
    if final_iter is not None:
        renderer = Rasterizer_simple(mesh_path=f"./raw_data/{vehicle}/vehicle.obj", angle=0, lgt_intensity=1,
        ...
        renderer = Rasterizer_simple(mesh_path=f"./raw_data/{vehicle}/vehicle.obj", angle=0, lgt_intensity=lgt_intensity)
        st = ShadowTracer(renderer.imesh.v_pos, renderer.imesh.t_pos_idx)
    ...
    for cam_idx, field in enumerate(fields(agent_input.cameras[3])):
        ...
        if torch.any(ret['depth'] > 0):
            ...
            bg_paths.append(f"/SSD_DISK_1/users/jiangjunzhe/2.NAVSIM/realengine/DriveX/data/nuplan/{token}/image_{cam_idx}/15.png")
    ...
    if final_iter is not None:
        ...
        save_image(shadow.reshape(*img_size, 3).permute(2, 0, 1), "shadow.png")
    else:
        st.precompute_rayintersect(resize, plane_normal, plane_point, c2car_ls[0][0], intrinsic_re[0], valid_mask=None)
        shadow = st.render_shadow()
        save_image(shadow.reshape(*resize, 3).permute(2, 0, 1), "shadow.png")

        # render_fg = partial(renderer.render_pbr, c2car_ls[0], intrinsic, img_size)
        render_fg = partial(renderer.render_pbr, intrinsic=intrinsic, img_size=img_size)
        run(opt, render_fn=render_fg, rast=renderer, ST=st)
