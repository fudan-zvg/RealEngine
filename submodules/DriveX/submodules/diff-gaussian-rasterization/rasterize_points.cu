	...
	const bool debug)
{
  ...
  torch::Tensor out_alpha = torch::full({3, H, W}, 0.0, float_opts);
  torch::Tensor out_semantic = torch::full({S, H, W}, 0.0, float_opts);
  torch::Tensor out_others = torch::full({1, H, W}, 0.0, float_opts); // distortion loss
  ...
  return std::make_tuple(rendered, out_color, out_depth, out_alpha, out_semantic, out_others, radii, geomBuffer, binningBuffer, imgBuffer);
}
