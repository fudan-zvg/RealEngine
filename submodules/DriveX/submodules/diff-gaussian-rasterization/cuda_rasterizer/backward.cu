...
renderCUDA(
	...
	const float* __restrict__ dL_dpixel_distortions,
...
)
{
	...
	if (inside) {
		...
		dL_dreg = dL_dpixel_distortions[pix_id];
	}
	...
	const float final_D = inside ? alphas[pix_id + H * W] : 0;
	const float final_D2 = inside ? alphas[pix_id + 2 * H * W] : 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Propagate gradients from distortion map
			float dL_dweight = 0;
...
void BACKWARD::render(
	...
	const float* dL_dpixel_distortions,
	float3* dL_dmean2D,
	float4* dL_dconic2D,
	float* dL_dopacity,
	float* dL_dcolors,
