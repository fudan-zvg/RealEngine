...
from submodules.DriveX.lib.utils.nuplan_utils import load_camera_info
from submodules.DriveX.lib.utils.data_utils import get_val_frames
from submodules.DriveX.lib.utils.colmap_utils import read_extrinsics_binary, qvec2rotmat

image_filename_to_cam = lambda x: int(x.split('/')[0].split('_')[1])  # image_{cam_id}/{frame}.png

def run_colmap_nuplan(result):
    ...
    for unqiue_cam in unique_cams:
        train_images_dir = os.path.join(colmap_dir, 'train_imgs', f'image_{unqiue_cam}')
        test_images_dir = os.path.join(colmap_dir, 'test_imgs', f'image_{unqiue_cam}')
        mask_images_dir = os.path.join(colmap_dir, 'mask', f'image_{unqiue_cam}')
    ...
    for i, image_filename in enumerate(image_filenames):
        ...
        new_image_filename = image_filename[-14:]
        c2w_dict[new_image_filename] = c2ws[i]
        mask_image_filenames.append(os.path.join(data_path, 'dynamic_mask', new_image_filename))
    ...
    for i, image_filename in enumerate(train_image_filenames):
        new_image_filename = os.path.join(train_images_dir, image_filename[-14:])
    ...
    for i, image_filename in enumerate(test_image_filenames):
        new_image_filename = os.path.join(test_images_dir, image_filename[-14:])
        if not os.path.exists(new_image_filename):
            shutil.copyfile(image_filename, new_image_filename)
    ...
    for i, image_filename in enumerate(mask_image_filenames):
        mask_images_dir = os.path.join(colmap_dir, 'mask')
        new_mask_filename = os.path.join(mask_images_dir, image_filename[-14:])
    ...
    _, extrinsics, _ = load_camera_info(cfg.source_path)
