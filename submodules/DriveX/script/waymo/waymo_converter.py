...
from submodules.DriveX.lib.utils.box_utils import bbox_to_corner3d, get_bound_2d_mask
from submodules.DriveX.lib.utils.img_utils import draw_3d_box_on_img
from submodules.DriveX.lib.utils.graphics_utils import project_numpy
    ...
    if 'lidar' in process_list:
        ...
        for frame_id, frame in tqdm(enumerate(datafile)):
            ...
            for laser_name, laser_name_str in laser_names_dict.items():
                ...
                pts_3d.append(np.concatenate([pcl[:, :3], pcl_attr[:, [1]]], axis=1))  # save LiDAR pointcloud in vehicle frame
...
def main():
    ...
    parser.add_argument('--root_dir', type=str, default='./data/waymo/waymo_format/validation')
    parser.add_argument('--save_dir', type=str, default='./data/waymo/streetgs_format/validation')
    parser.add_argument('--track_file', type=str, default='/nas/home/yanyunzhi/waymo/castrack/seq_infos/val/result.json')
    parser.add_argument('--split_file', type=str, default=None)
    parser.add_argument('--segment_file', type=str, default='./script/waymo/waymo_splits/segment_list_val.txt')
    ...
    seq_all_lists = open(segment_file).read().splitlines()
    seq_lists = os.listdir(root_dir)
    seq_lists = sorted(seq_lists)
    seq_lists = [seq.split('.')[0] for seq in seq_lists]
    scene_ids_list = list(range(min(12, len(seq_lists))))
    ...
    for i, scene_id in enumerate(scene_ids_list):
        ...
        seq_save_dir = os.path.join(save_dir, f"{seq_all_lists.index(seq_lists[scene_id]):03d}")
