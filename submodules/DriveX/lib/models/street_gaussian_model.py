...
from submodules.DriveX.lib.utils.general_utils import quaternion_to_matrix, \
    ...
    quaternion_invert
from submodules.DriveX.lib.utils.graphics_utils import BasicPointCloud
from submodules.DriveX.lib.utils.system_utils import mkdir_p
from submodules.DriveX.lib.utils.sh_utils import SH2RGB
from plyfile import PlyData, PlyElement
from submodules.DriveX.lib.models.gaussian_model import GaussianModel
from submodules.DriveX.lib.models.gaussian_model_bkgd import GaussianModelBkgd
from submodules.DriveX.lib.models.gaussian_model_actor import GaussianModelActor
from submodules.DriveX.lib.models.gaussian_model_sky import GaussinaModelSky
from bidict import bidict
from submodules.DriveX.lib.utils.camera_utils import Camera
from submodules.DriveX.lib.utils.sh_utils import eval_sh
from submodules.DriveX.lib.utils.general_utils import save_ply
from submodules.DriveX.lib.models.actor_pose import ActorPose
from submodules.DriveX.lib.models.sky_cubemap import SkyCubeMap
from submodules.DriveX.lib.models.color_correction import ColorCorrection
from submodules.DriveX.lib.models.camera_pose import PoseCorrection
...
class StreetGaussianModel(nn.Module):
    ...
    def save_ply(self, path):
        ...
        for i in range(self.models_num):
            ...
            save_ply(model.get_xyz, path.replace("point_cloud.ply", f"{model_name}.ply"),
                     SH2RGB(model.get_features[:, 0]).clamp(0, 1), max_points=1e6)
    ...
    def get_xyz(self):
        ...
        if self.use_pose_correction:
            xyzs = self.pose_correction.correct_gaussian_xyz(self.viewpoint_camera, xyzs)
    ...
    def update_learning_rate(self, iteration, exclude_list=[], exclude_attributes=[]):
        for model_name in self.model_name_id.keys():
            if startswith_any(model_name, exclude_list):
                continue
            model: GaussianModel = getattr(self, model_name)
        ...
        if self.actor_pose is not None and "actor_pose" not in exclude_attributes:
            self.actor_pose.update_learning_rate(iteration)

        if self.sky_cubemap is not None and "sky_cubemap" not in exclude_attributes:
    ...
    def update_optimizer(self, exclude_list=[], exclude_attributes=[]):
        for model_name in self.model_name_id.keys():
            if startswith_any(model_name, exclude_list):
                continue
            model: GaussianModel = getattr(self, model_name)
