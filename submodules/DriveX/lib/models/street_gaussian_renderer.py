...
from submodules.DriveX.lib.utils.camera_utils import Camera, make_rasterizer
...
class StreetGaussianRenderer:
    ...
    def render(
        ...
        result = self.render_kernel(viewpoint_camera, pc, convert_SHs_python, compute_cov3D_python, scaling_modifier, override_color, overrided_shs=overrided_shs)
    ...
    def render_kernel(
        ...
        scaling_modifier = scaling_modifier or self.cfg.render.scaling_modifier
        rasterizer = make_rasterizer(viewpoint_camera, pc.max_sh_degree, bg_color, scaling_modifier)

        convert_SHs_python = convert_SHs_python or self.cfg.render.convert_SHs_python
        compute_cov3D_python = compute_cov3D_python or self.cfg.render.compute_cov3D_python
        ...
        rendered_color, radii, rendered_depth, rendered_acc, rendered_feature, rendered_dist = rasterizer(
            means3D=means3D,
            means2D=means2D,
            opacities=opacity,
            shs=shs,
