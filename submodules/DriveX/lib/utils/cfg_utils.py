import os
import numpy as np
from submodules.DriveX.lib.config import yacs
...
def parse_cfg(cfg, args):
    ...
    if args.exp_name is not None:
        cfg.model_path = f"{cfg.model_path}-{args.exp_name}"
    # if args.debug:
    #     cfg.model_path = f"{cfg.model_path}-debug"

    if args.token is not None:
        cfg.source_path = os.path.join(cfg.source_path, args.token)
