from submodules.DriveX.lib.utils.nuplan_utils import generate_dataparser_outputs
from submodules.DriveX.lib.utils.graphics_utils import focal2fov, BasicPointCloud
from submodules.DriveX.lib.utils.data_utils import get_val_frames
from submodules.DriveX.lib.datasets.base_readers import CameraInfo, SceneInfo, getNerfppNorm, fetchPly, get_PCA_Norm, get_Sphere_Norm
...
def readNuplanInfo(path, cfg, images='images', split_train=-1, split_test=-1, **kwargs):
    ...
    if not os.path.exists(lidar_depth_dir):
        cmd = f'python script/nuplan/generate_lidar_depth.py --datadir {path}'
    ...
    for i in tqdm(range(len(ixts))):
        ...
        # load lidar depth
        if load_lidar_depth:
            depth_path = os.path.join(path, 'lidar_depth', f'{image_name}_{cam}.npy')
        ...
        # load sky mask
        if load_sky_mask:
            sky_mask_path = os.path.join(image_path[:-15], f'mask_{cam}', f'{image_name}_sky.png')
        ...
        cam_info = CameraInfo(
            ...
            image=image, image_path=image_path, image_name=f'{image_name}_{cam}',
