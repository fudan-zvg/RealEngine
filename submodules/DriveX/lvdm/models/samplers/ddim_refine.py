...
class DDIMRefineSampler(object):
    ...
    def sample(self,
        ...
        if sds:
            loss, target = self.score_distillation_sampling(conditioning, size,
                                                    callback=callback,
    ...
    def ddim_sampling(self, cond, shape,
        ...
        z_gaussians = kwargs.get('z_gaussians', None)
        ...
        if strength is not None:
            start_i = min(round(total_steps * (1-strength)), total_steps-1)
            img = self.model.q_sample(z_gaussians, torch.full((b,), time_range[start_i], device=device, dtype=torch.long))
        ...
        for i, step in enumerate(iterator):
            if strength is not None and i < start_i:
                continue
    ...
    @torch.no_grad()
    def score_distillation_sampling(self, cond, shape,
                      x_T=None, ddim_use_original_steps=False,
                      callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, log_every_t=100,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
        ...
        if strength is not None:
            start_i = min(round(total_steps * (1-strength)), total_steps-1)
            noise = torch.randn_like(z_gaussians)
            img = self.model.q_sample(z_gaussians, torch.full((b,), time_range[start_i], device=device, dtype=torch.long), noise=noise)
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            model_output = self.model.apply_model(img, ts, cond, **kwargs) # unet denoiser
        else:
            ### do_classifier_free_guidance
            if isinstance(cond, torch.Tensor) or isinstance(cond, dict):
                e_t_cond = self.model.apply_model(img, ts, cond, **kwargs)
                e_t_uncond = self.model.apply_model(img, ts, unconditional_conditioning, **kwargs)
            else:
                raise NotImplementedError
        ...
        if self.model.parameterization == "v":
            noise_pred = self.model.predict_eps_from_z_and_v(img, ts, model_output)
        else:
            noise_pred = model_output

        grad = (1-self.model.alphas_cumprod.gather(-1,ts)) * (noise_pred - noise)
        grad = torch.nan_to_num(grad)
        
        target = (z_gaussians - grad).detach()
        
        # current prediction for x_0
        assert self.model.parameterization == "v"
        pred_x0 = self.model.predict_start_from_z_and_v(img, ts, model_output)
        
        return target, pred_x0
