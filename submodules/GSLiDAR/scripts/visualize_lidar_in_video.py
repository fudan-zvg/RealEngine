...
def rgb_to_hex(color):
    ...
    for idx in range(color.shape[0]):
        r, g, b = (color[idx] * 255).astype(int)
        hex_color.append("#{:02x}{:02x}{:02x}".format(r, g, b))

    return hex_color


def tensor_list_to_video(tensor_list, output_path, fps=30, scale_factor=1):
    # 确保每个 Tensor 是 [C, H, W]，通常 C = 3 (RGB)
    h, w = tensor_list[0].shape[1], tensor_list[0].shape[2]  # 获取高度和宽度

    # 创建视频写入器
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 编码格式
    video_writer = cv2.VideoWriter(os.path.join(output_path, '2d.mp4'), fourcc, fps, (w * scale_factor, h * scale_factor))

    # 定义转换为 NumPy 和调整通道的函数
    to_pil_image = transforms.ToPILImage()

    os.makedirs(os.path.join(output_path, '2d'), exist_ok=True)

    for idx, tensor in tqdm(enumerate(tensor_list)):
        save_image(tensor, os.path.join(output_path, '2d', f'{idx:03d}.png'))
        ...
        text = ["GT", "GS-LiDAR", "GT", "GS-LiDAR"]
        color = [(0, 0, 0), (0, 0, 0), (255, 255, 255), (255, 255, 255)]
        for idx in range(4):
            cv2.putText(frame_bgr, text[idx], (10 * scale_factor, (66 * idx + 60) * scale_factor),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.3 * scale_factor, (255, 255, 255), int(np.sqrt(scale_factor)), cv2.LINE_AA)

        watermark_text = "ICLR Submission 2018: GS-LiDAR"
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(51)):
        if os.path.exists(os.path.join(root_path, "train_refine_render", f"{i:03d}_depth.png")):
            mode = "train"
        elif os.path.exists(os.path.join(root_path, "test_refine_render", f"{i:03d}_depth.png")):
            mode = "test"
        else:
            raise FileNotFoundError

        depth_gt = Image.open(os.path.join(root_path, f"{mode}_refine_render", f"{i:03d}_depth_gt.png"))
        depth = Image.open(os.path.join(root_path, f"{mode}_refine_render", f"{i:03d}_depth.png"))
        intensity_gt = Image.open(os.path.join(root_path, f"{mode}_refine_render", f"{i:03d}_intensity_gt.png"))
        intensity = Image.open(os.path.join(root_path, f"{mode}_refine_render", f"{i:03d}_intensity.png"))

        transform = transforms.ToTensor()
        grid = [transform(image) for image in [depth_gt, depth, intensity_gt, intensity]]
        grid = make_grid(grid, nrow=1, padding=0)
        video_frame.append(grid)
    ...
    tensor_list_to_video(video_frame, output_path, fps=10, scale_factor=2)
