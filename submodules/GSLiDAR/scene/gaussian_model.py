...
import torch
import numpy as np
from submodules.GSLiDAR.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation, get_step_lr_func
from torch import nn
import os
from plyfile import PlyData, PlyElement
from submodules.GSLiDAR.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from submodules.GSLiDAR.utils.graphics_utils import BasicPointCloud
from submodules.GSLiDAR.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        features = torch.zeros((fused_color.shape[0], 4, self.get_max_sh_channels)).float().cuda()  # r g b ray_drop_probability
        features[:, :3, 0] = fused_color

        if self.random_init_point > 0:
            ## random up and far
            r_max = 100000
            r_min = 2
        ...
        features[:, 3:4, 0] = RGB2SH(0.01)  # 初始每个点ray drop概率为小量
        ...
        opacities = inverse_sigmoid(self.init_opa * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
    ...
    def training_setup(self, training_args):
        self.dynamic = training_args.dynamic
    ...
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity) * self.init_opa))
    ...
    def prune_points(self, mask):
        ...
        if 'velocity' not in optimizable_tensors.keys():
            self._velocity = nn.Parameter(self._velocity[valid_points_mask].requires_grad_(True))
        else:
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
        ...
        self._scaling_t = optimizable_tensors['scaling_t']
        if 'velocity' not in optimizable_tensors.keys():
            self._velocity = nn.Parameter(torch.cat((self._velocity, new_velocity), dim=0).requires_grad_(True))
        else:
            self._velocity = optimizable_tensors['velocity']
    ...
    def densify_and_prune(self, max_grad, max_grad_abs, min_opacity, extent, max_screen_size, max_grad_t=None, prune_only=False):
        ...
        if not prune_only:
            ...
            if self.t_grad:
                ...
                # self.densify_and_split(grads_abs, max_grad_abs, extent, grads_t, max_grad_t, time_split=True)
            else:
                ...
                # self.densify_and_split(grads_abs, max_grad_abs, extent, grads_t, max_grad_t, time_split=False)
