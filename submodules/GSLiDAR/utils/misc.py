...
def point_removal(pc_raw):
    pc_rm = pc_raw.detach().cpu().numpy()
    pc_rm = pc_rm @ np.array([1, 0, 0,
                              0, 0, -1,
                              0, 1, 0]).reshape(3, 3)
    pc_rm = range_filter(pc_rm)
...
def pointList_from_cams(cameras, args):
    point_list = {}

    for c in tqdm(cameras):
        idx = c.colmap_id % args.frames
        if idx not in point_list.keys():
            point_list[idx] = loadPoint(args, c)
        else:
            point_list[idx] = np.concatenate([point_list[idx], loadPoint(args, c)])

    return point_list


def loadPoint(args, camera):
    gt_lidar = pano_to_lidar(camera.pts_depth, vfov=args.vfov, hfov=args.hfov)
    gt_lidar = gt_lidar.detach().cpu().numpy()
    point_w = (gt_lidar - camera.T) @ camera.R.transpose()
    return point_w
