...
__device__ const float near_n = 3.0;
__device__ const float far_n = 200.0;
...
__forceinline__ __device__ void getRect_bias(const float2 aabb_min, const float2 aabb_max, uint2 &rect_min, uint2 &rect_max, dim3 grid)
{
	rect_min = {
		min(grid.x, max((int)0, (int)(aabb_min.x / BLOCK_X))),
		min(grid.y, max((int)0, (int)(aabb_min.y / BLOCK_Y)))};
	rect_max = {
		min(grid.x, max((int)0, (int)((aabb_max.x + BLOCK_X - 1) / BLOCK_X))),
		min(grid.y, max((int)0, (int)((aabb_max.y + BLOCK_Y - 1) / BLOCK_Y)))};
}

__forceinline__ __device__ void getRect_panorama(const float2 p, int max_radius, int2 &rect_min, int2 &rect_max, dim3 grid)
{
	rect_min = {
...
__forceinline__ __device__ bool in_frustum_panorama(
	float3 mean3d,
	float VFOV_min,
	float VFOV_max,
	float HFOV_min,
...
{
	// Bring points to screen space
	float center_v = (VFOV_max + VFOV_min) / 2;
	float half_range_v = (VFOV_max - VFOV_min) / 2;
	float ratio_v = abs((mean3d.x - center_v) / half_range_v);

	float center_h = (HFOV_max + HFOV_min) / 2;
	float half_range_h = (HFOV_max - HFOV_min) / 2;
	float ratio_h = abs((mean3d.y - center_h) / half_range_h);

	if (mean3d.z <= near_n * scale_factor || ratio_v > 1.3 || ratio_h > 1.3)
	{
		return false;
	}
	return true;
