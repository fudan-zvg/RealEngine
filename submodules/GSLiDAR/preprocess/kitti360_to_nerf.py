...
def main():
    ...
    range_view_paths = range_view_paths + range_view_paths

    # Get lidar2world.
    lidar2world = k3.load_lidars(sequence_name, frame_ids)
    lidar2world = np.concatenate([lidar2world, lidar2world], axis=0)
    ...
    for split, indices in split_to_all_indices.items():
        ...
        Ks_split = [-1 for i in indices]  # [Ks[i] for i in indices]
        Ts_split = [-1 for i in indices]  # [Ts[i] for i in indices]
        ...
        json_dict = {
            ...
            "w_lidar": lidar_w,
            "h_lidar": lidar_h,
            "fl_x": -1,  # Ks_split[0][0, 0],
            "fl_y": -1,  # Ks_split[0][1, 1],
            "cx": -1,  # Ks_split[0][0, 2],
            "cy": -1,  # Ks_split[0][1, 2],
            "aabb_scale": 2,
            "frames": [
