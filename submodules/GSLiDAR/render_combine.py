...
def render_all_map(cams, gaussians, renderFunc, renderArgs, env_map, foreground_pc):
    ...
    for idx, viewpoint in enumerate(cams):
        render_pkg = renderFunc(viewpoint, gaussians, *renderArgs, env_map=env_map, foreground_pc=foreground_pc)
...
@torch.no_grad()
def render_novel_view(ego_pose, gaussians: GaussianModel, renderFunc, renderArgs, env_map=None,
                      foreground_pc: list[tuple[car_GaussianModel, torch.Tensor]] = None):
    # calculate cams
    print("Loading camera poses ...")
    cams = calculate_poses(ego_pose)

    print("Rendering ...")
    all_map = render_all_map(cams, gaussians, renderFunc, renderArgs, env_map=env_map, foreground_pc=foreground_pc)
    ...
    outdir = os.path.join(args.model_path, "render_novel_view")
    save_image(grid, os.path.join(outdir, f"{0:03d}.png"))
...
if __name__ == "__main__":
    ...
    # load car gaussian model
    print("Loading Car Gaussian model ...")
    car_gaussians = car_GaussianModel(args)
    car_gaussians.training_setup(args)
    car_checkpoints = glob.glob(os.path.join(args.car_path, "ckpt", "chkpnt*.pth"))
    assert len(car_checkpoints) > 0, "No checkpoints found."
    car_checkpoint = sorted(car_checkpoints, key=lambda x: int(x.split("chkpnt")[-1].split(".")[0]))[-1]
    (model_params, first_iter) = torch.load(car_checkpoint)
    car_gaussians.restore(model_params, args)
    R, T, S = torch.load(os.path.join(args.car_path, "ckpt", "transform_pcd_2_lidar.pt"))
    ...
    car_gaussians.transfer_gaussian_points(R, T + torch.tensor([0, 0, 0.11]).cuda(), S * args.scale_factor)

    # car_l2w = np.loadtxt(os.path.join(args.source_path, 'pose', '35.txt'))
    # car_l2w = np.diag(np.array([1 / scale_factor] * 3 + [1])) @ transform @ car_l2w
    # car_l2w[:3, 3] *= scale_factor
    # car_l2w = torch.from_numpy(car_l2w).float().cuda()
    # car_l2w_2 = car_l2w.clone()
    # car_l2w_2[1, 3] += 3 * args.scale_factor
    car_l2w = torch.tensor([[4.3685e-01, 8.9953e-01, -3.8820e-03, 1.7528e-01],
                            [8.9950e-01, -4.3687e-01, -6.8449e-03, 7.4811e-01],
                            [-7.8530e-03, -5.0167e-04, -9.9997e-01, 1.2404e-01],
                            [0.0000e+00, 0.0000e+00, 0.0000e+00, 1.0000e+00]], device='cuda:0')
    car_l2w_2 = torch.tensor([[-0.9450, -0.3272, -0.0039, 0.5843],
                              [-0.3271, 0.9450, -0.0068, 0.4056],
                              [0.0059, -0.0052, -1.0000, 0.1248],
                              [0.0000, 0.0000, 0.0000, 1.0000]], device='cuda:0')
    car_l2w_3 = torch.tensor([[-0.9450, -0.3272, -0.0039, 0.9302],
                              [-0.3271, 0.9450, -0.0068, 0.5253],
                              [0.0059, -0.0052, -1.0000, 0.1226],
                              [0.0000, 0.0000, 0.0000, 1.0000]], device='cuda:0')

    # novel view
    # 注意！ego_pose 第0帧的translation为(0, 0, 0)
    # ego_pose = np.loadtxt(os.path.join(args.source_path, 'pose', '00.txt'))
    ego_pose = np.array([[8.90472926e-01, 4.55019868e-01, 3.85839287e-03, 5.94682579e+00],
                         [-4.55034441e-01, 8.90413669e-01, 1.03515504e-02, -4.21682847e+00],
                         [1.27459535e-03, -1.09734770e-02, 9.99938977e-01, -6.22347545e-02],
                         [0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 1.00000000e+00]])

    # (N, 4): x y z intensity
    points = render_novel_view(ego_pose, gaussians, render, (args, background), env_map=(env_map, lidar_raydrop_prior),
                               foreground_pc=[(car_gaussians, car_l2w), (car_gaussians, car_l2w_2), (car_gaussians, car_l2w_3)])
