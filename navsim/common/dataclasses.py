...
class EgoStatus:
    ...
    ego2global_translation: npt.NDArray[np.float64] = None
    ego2global_rotation: npt.NDArray[np.float64] = None
    ...
    def from_scene_dict_list(
        ...
        for frame_idx in range(num_history_frames):
            ...
            ego_status = EgoStatus(
                ...
                ego2global_translation=scene_dict_list[frame_idx]["ego2global_translation"],
                ego2global_rotation=scene_dict_list[frame_idx]["ego2global_rotation"],
            )
...
class Scene:
    ...
    def _build_ego_status(cls, scene_frame: Dict) -> EgoStatus:
        ...
        return EgoStatus(
            ...
            ego2global_rotation=ego_quaternion.rotation_matrix,
            ego_velocity=np.array(ego_dynamic_state[:2], dtype=np.float32),
            ego_acceleration=np.array(ego_dynamic_state[2:], dtype=np.float32),
