...
def convert_relative_to_absolute(points_rel, origin: StateSE2):
    theta = -origin.heading
    origin_array = np.array([[origin.x, origin.y, origin.heading]], dtype=np.float64)

    R = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]])
    points_abs = np.zeros_like(points_rel)
    points_abs[..., 2] = normalize_angle(points_rel[..., 2])
    points_abs[..., :2] = points_rel[..., :2] @ R
    points_abs = points_abs + origin_array
