...
CONFIG_NAME = "default_run_pdm_score_with_render_base"
...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[List[Dict[str, Any]]]:
    """
    ...
    openloop_results: List[Dict[str, Any]] = []
    ...
    for idx, (token, object_adds) in enumerate(cfg.eval_tokens_info):
        ...
        # for vis
        vis_file_name = os.path.join(os.getenv('NAVSIM_EXP_ROOT'), agent.name(), "base", token)
        ...
        score_row_openloop: Dict[str, Any] = {"token": token, "valid": True}
        score_row_human: Dict[str, Any] = {"token": token, "valid": True}
        try:
            ...
            for time_idx in tqdm(range(num_iter + 1)):
                ...
                if trajectory_init is None:
                    ...
                    trajectory_openloop = copy.deepcopy(trajectory)
                else:
            ...
            # open loop
            openloop_result = pdm_score(
                metric_cache=metric_cache,
                model_trajectory=trajectory_openloop,
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
                scorer=scorer,
            )
            print("open-loop\n", openloop_result)
            score_row_openloop.update(asdict(openloop_result))
            ...
            # human
            trajectory_init.poses = convert_absolute_to_relative_se2_array(StateSE2(*scene.frames[3].ego_status.ego_pose),
                                                                           np.array([scene.frames[4 + i].ego_status.ego_pose for i in range(num_iter)]))
            human_result = pdm_score(
                metric_cache=metric_cache,
                model_trajectory=trajectory_init,
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
            ...
            print("human\n", human_result)
            score_row_human.update(asdict(human_result))
            ...
            score_row_openloop["valid"] = False
            score_row_human["valid"] = False

        pdm_results.append(score_row)
        openloop_results.append(score_row_openloop)
        human_results.append(score_row_human)
    return [pdm_results, openloop_results, human_results]
...
def main(cfg: DictConfig) -> None:
    ...
    list_score_rows: List[Tuple[Dict[str, Any], int, int]] = worker_map(worker, run_pdm_score, data_points)

    for simulate_type, score_rows in zip(["closed-loop", "open-loop", "human"], list_score_rows):
        ...
        pdm_score_df.to_csv(save_path / f"{simulate_type}.csv")

        logger.info(
            f"""
            Finished running {simulate_type} evaluation.
                Number of successful scenarios: {num_sucessful_scenarios}.
                Number of failed scenarios: {num_failed_scenarios}.
                Final average score of valid results: {pdm_score_df['score'].mean()}.
                Results are stored in: {save_path / f"{simulate_type}.csv"}.
            """
        )
