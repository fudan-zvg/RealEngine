        ...
        for idx, viewpoint in enumerate(cams):
            if init_obj2globals is not None:
                ...
                for i, obj_model in enumerate(init_obj2globals):
                    ...
                    # car_l2w[0,3] -= 1.0*frame_idx #front
                    # car_l2w[1,3] -= 1.0*frame_idx # side
...
def update_agent_input_edit_side(new_pose, scene, agent_input, img_render_edit_model=None, lidar_render_edit_model=None, 
    ...
    if hasattr(agent_input, 'cameras') and img_render_edit_model is not None:
        ...
        if update_all:
            for i in range(len(updated_stat)):
                new_camera = RenderCameras.from_render_edit_side(
    ...
    if hasattr(agent_input, 'lidars') and lidar_render_edit_model is not None:
        ...
        if update_all:
            for i in range(len(updated_stat)):
                new_lidar = RenderLidars.from_render_edit_side(
