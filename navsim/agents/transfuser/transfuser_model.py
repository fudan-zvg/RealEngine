...
class TransfuserModel(nn.Module):
    ...
    def forward(self, features: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        bev_feature = self._bev_downscale(bev_feature).flatten(-2, -1) #4, 64, 256, 1,256,8,8
        bev_feature = bev_feature.permute(0, 2, 1)
        status_encoding = self._status_encoding(status_feature) #4, 256

        keyval = torch.concatenate([bev_feature, status_encoding[:, None]], dim=1) #4,65,256
