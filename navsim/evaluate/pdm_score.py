...
def pdm_score(
    ...
    # 第0个不是human，是PDM Closed算的最大安全轨迹
    pred_idx = 1
...
def pdm_score_multi(
        metric_cache: MetricCache,
        model_trajectorys: List[Trajectory],
        future_sampling: TrajectorySampling,
        simulator: PDMSimulator,
        scorer: PDMScorer
) -> np.array:
    ...
    pdm_trajectory = metric_cache.trajectory
    pred_trajectorys = [transform_trajectory(model_trajectory, initial_ego_state) for model_trajectory in model_trajectorys]

    pdm_states, pred_states = (
        get_trajectory_as_array(pdm_trajectory, future_sampling, initial_ego_state.time_point),
        [get_trajectory_as_array(pred_trajectory, future_sampling, initial_ego_state.time_point) for pred_trajectory in pred_trajectorys],
    )
