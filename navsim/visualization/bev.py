...
def add_configured_bev_on_ax_edit(ax: plt.Axes, map_api: AbstractMap, frame: Frame, offset: Optional[npt.NDArray[np.float32]], edit: bool = False) -> plt.Axes:
    """
    Adds birds-eye-view visualization optionally with map, annotations, or lidar
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
    ...
    if edit:
        edit_annotations = Annotations(boxes=[np.array([offset[0], offset[1], 0, 4.588423252105713, 1.9402943849563599, 1.5295588970184326, offset[2]])],
                                       names=['vehicle'],
                                       velocity_3d=np.empty((1, 1)),
                                       instance_tokens=['1'],
                                       track_tokens=['1'])
        if "annotations" in BEV_PLOT_CONFIG["layers"]:
            add_annotations_to_bev_ax(ax, edit_annotations)
    else:
        if "annotations" in BEV_PLOT_CONFIG["layers"]:
            add_annotations_to_bev_ax(ax, frame.annotations)
...
def add_configured_bev_on_ax_edit_unify(ax: plt.Axes, map_api: AbstractMap, frame: Frame, edit_annotations: Annotations) -> plt.Axes:
    """
    Adds birds-eye-view visualization optionally with map, annotations, or lidar
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
...
def add_configured_bev_on_ax_edit_static(ax: plt.Axes, map_api: AbstractMap, frame: Frame, edit_annotations: Annotations, agent_ego_pose, carfootprint=None) -> plt.Axes:
    """
    Adds birds-eye-view visualization optionally with map, annotations, or lidar
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
    ...
    if "map" in BEV_PLOT_CONFIG["layers"]:
        add_map_to_bev_ax(ax, map_api, StateSE2(*agent_ego_pose))

    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax(ax, edit_annotations, carfootprint=carfootprint)
...
def add_configured_bev_on_ax_agent(ax: plt.Axes, map_api: AbstractMap, agent_ego: EgoStatus, scene_frame: Frame) -> plt.Axes:
    """
    Adds birds-eye-view visualization optionally with map, annotations, or lidar
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
    ...
    :return: ax with plot
    l=4.588423252105713
    w=1.9402943849563599
    h=1.5295588970184326
    vehicle
    """

    if "map" in BEV_PLOT_CONFIG["layers"]:
        add_map_to_bev_ax(ax, map_api, StateSE2(*agent_ego))

    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax(ax, scene_frame.annotations)

    if "lidar" in BEV_PLOT_CONFIG["layers"]:
        add_lidar_to_bev_ax(ax, scene_frame.lidar)

    return ax


def add_annotations_to_bev_ax(ax: plt.Axes, annotations: Annotations, add_ego: bool = True, carfootprint: CarFootprint = None) -> plt.Axes:
    """
    ...
    if add_ego and carfootprint is None:
        ...
        car_footprint = CarFootprint.build_from_rear_axle(
            rear_axle_pose=StateSE2(0, 0, 0),
            vehicle_parameters=carfootprint.vehicle_parameters,
        )
        add_oriented_box_to_bev_ax(
            ax, car_footprint.oriented_box, AGENT_CONFIG[TrackedObjectType.EGO], add_heading=False
        )
...
def get_pacifica_parameters_edit(cfg) -> VehicleParameters:
    """
...
def add_trajectory_to_bev_ax_multi_agent(ax: plt.Axes, trajectory: Trajectory, config: Dict[str, Any]) -> plt.Axes:
    """
    Add trajectory poses as lint to plot
    :param ax: matplotlib ax object
    :param trajectory: navsim trajectory dataclass
